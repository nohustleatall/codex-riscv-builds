# Codex 0.130.0 on Orange Pi RV2
...
Codex 0.130.0 builds with the same local RISC-V compatibility changes used for the earlier Orange Pi RV2 releases:

1. Build OpenSSL from source for `riscv64gc-unknown-linux-gnu`.
2. Use `seccompiler`'s `riscv64` target architecture when installing the network seccomp filter.
